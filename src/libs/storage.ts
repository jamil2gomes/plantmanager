...
export async function savePlant(plant: PlantProps): Promise<void> {
    try {
        ...
        const notificationId = Notifications.scheduleNotificationAsync({
            content: {
                title: 'Heeeey, 🌱',
                body: `Está na hora de regar a plantinha ${plant.name}!`,
                sound: true,
...
export async function removePlants(id: string): Promise<void> {
    ...
    try {
        ...
        await Notifications.cancelScheduledNotificationAsync(plantas[id].notificationId);

        delete plantas[id];

        await AsyncStorage.setItem('@plantmanager:plants', JSON.stringify(plantas));
